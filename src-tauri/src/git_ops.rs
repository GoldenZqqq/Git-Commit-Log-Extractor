use crate::models::{CommitRecord, GitIdentity, RepoInfo, RepoScanProgress, RepoScanResult};
...
const MAX_SCAN_WARNING_DETAILS: usize = MAX_SCAN_WARNINGS - 1;
...
impl ScanWarningCollector {
    fn push(&mut self, path: &Path, operation: &str, error: &io::Error) {
        let message = format!(
            "扫描已跳过“{}”：{}失败（{}）",
            display_path(path),
            operation,
...
mod tests {
    ...
    fn scan_result_warns_when_root_is_not_a_directory_and_keeps_valid_repos() {
        ...
        assert!(
            result
                .warnings
                .iter()
                .any(|warning| warning.contains("not-a-directory")
                    && warning.contains("读取目录失败"))
        );
    ...
    fn scan_result_warns_for_broken_directory_link_and_keeps_valid_repos() {
        ...
        #[cfg(windows)]
        let link_result =
            std::os::windows::fs::symlink_dir(root.join("missing-target"), &broken_link);
