...
- [ ] AC1：8 个子任务全部归档为 `completed`，父任务进度为 8/8。
- [ ] AC2：每个子任务均有独立的验证证据、提交记录和远端 push 证据。
- [ ] AC3：`main` 包含 v0.5.3 已发布能力，发布只能从受控主线执行。
- [ ] AC4：升级、规模、支持与桌面质量门禁覆盖关键失败路径。
- [ ] AC5：前端与 Rust 超大模块完成边界拆分，核心源码文件满足项目 600 行上限或有经批准且可执行的剩余拆分任务。
- [ ] AC6：下一产品主线有证据、范围、非目标和进入实现的明确门槛。
